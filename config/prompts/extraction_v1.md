# Stage F extraction prompt, version 1

**Version 1, 12 August 2026.** Hash this file into `data/batch_ledger.csv`
...
`robins_i` for a non-randomised effect. Never assess under two speculatively. Every
domain of the chosen instrument is required, each with a `rationale` and a
`source_location`. Leave `checker` blank: that is Stage G's, and it is a human's.
